...
def calOrientation(msg):
    global roll, pitch, yaw, orientation_current
    #the data recieved from the sensor in in quaternion form
    orientation_current = [ msg.orientation.x, msg.orientation.y, msg.orientation.z, msg.orientation.w]
    #So, we need to convert that data from quaternion to euler using an in-built function
    roll, pitch, yaw = euler_from_quaternion(orientation_current)
