...
from geometry_msgs.msg import Vector3Stamped #it basically gives you the velocity vector from which you need to extract the data
...
def calPosition(pos):
    ...
    altitude = round(pos.pose[1].position.z,3)
...
def alt_control(vel, imu):
    ...
    # calAltitude(gps) #commented due to debugging purposes
...
def control():
    ...
    #these topics are already present in the given topics 
    #for example topics like /firefly/gps always publishes the message in the form of NavSatfix
    #How do we know that?
    #just open the terminal once you start the simulation and type rostopic lists
    #then after viewing the names just type_in rostopic info <name>
    #this will give you the message it is publishing or subscribed to
    #and then do rosmsg info <message_name>
    # gps_sub = message_filters.Subscriber("/firefly/gps", NavSatFix) 
    vel_sub = message_filters.Subscriber("/firefly/ground_speed", TwistStamped)
    imu_sub = message_filters.Subscriber("/firefly/imu", Imu)
    # odo_sub = message_filters.Subscriber("/firefly/odometry_sensor1/odometry", Odometry)
    ts = message_filters.TimeSynchronizer([vel_sub, imu_sub], 2)
